...
const endpointId = '3kycmn2drm6u2e';  // Correct endpoint ID

async function testRunpodApi() {
  try {
    console.log(`Testing RunPod API with endpoint: ${endpointId}`);
    ...
    while (!completed && attempts < 30) {
      ...
      if (statusResponse.data.status === 'COMPLETED') {
        ...
        if (statusResponse.data.output && statusResponse.data.output.images) {
          console.log(`Generated ${statusResponse.data.output.images.length} images`);
        }
        completed = true;
